  ...
  return (
    <header className="bg-gray-800 text-white py-6 mb-8">
      <div className="container mx-auto px-4">
        <h1 className="text-3xl font-bold">Cluster Search</h1>
        <p className="mt-2 text-gray-300">Find similar image clusters in our database</p>
      </div>
    </header>
  );
}
...
function ClusterSearch() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('https://atk.dbackup.cloud/search_cluster', {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Search image clusters" />
      </Head>
      <Header />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
              {searchResults.map((result, index) => (
                <div key={index} className="text-center">
                  <Link href={`/cluster?filename=${result.entity.representative_file_name}`} legacyBehavior>
                    <a className="block">
                      <Image
                        loader={imageLoader}
                        src={result.entity.representative_file_name}
                        alt={`Cluster ${index + 1}`}
                        width={300}
                        height={300}
                  ...
                  <p className="text-sm text-gray-600">File Count: {result.entity.file_count}</p>
                </div>
              ))}
            </div>
          </div>
